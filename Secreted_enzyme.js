...
fs.appendFileSync(filn, "prodecay_chance\t"+prodecay_chance+"\n"+"prodecay_interval\t"+prodecay_interval+"\n"+"pro_decay\t"+pro_decay+"\n") 
...
filn = dir+"/Product_"+"productionganger_"+filn_suffix+".txt"	
if(fs.existsSync(filn)){
	fs.unlinkSync(filn)
} 
...
grid_dir = dir+"/GridStates_"+"productionganger_"+filn_suffix+"/"	
if (!fs.existsSync(grid_dir)){
    fs.mkdirSync(grid_dir);
} 
    ...
    function cacatoo() {
       ...
       sim.makeGridmodel("substrate") 
       sim.makeGridmodel("production") 
 ...
 //           console.log(i,j)
   //         console.log(sim.cells.grid[i][j].alive)
     //       console.log()               
       ...
       sim.substrate.initialise = function () {
            for (let i = 0; i < sim.substrate.nc; i++) for (let j = 0; j < sim.substrate.nr; j++) { // This is a for loop that is executed when the initial value i < sim.cells.nc and value j < sim.cells.nr. The range of the loop is "0~ncol" and "0~nrow", so the loop traverses the entire model. 
                sim.substrate.grid[i][j].substratesarray = new Array(num_CG).fill(0) 
                if(sim.cells.grid[i][j].alive == 1 ){
                    sim.substrate.grid[i][j].substratesarray = new Array(num_CG).fill(Math.floor((Math.random()*maxInitsubstrateNum)+1)) 
                } 
            } 
       } 

       sim.production.initialise = function () {
            for (let i = 0; i < sim.production.nc; i++) for (let j = 0; j < sim.production.nr; j++) { // This is a for loop that is executed when the initial value i < sim.cells.nc and value j < sim.cells.nr. The range of the loop is "0~ncol" and "0~nrow", so the loop traverses the entire model. 
                sim.production.grid[i][j].productionsarray = new Array(num_CG).fill(0)  
            } 
       } 

       sim.CGtypes = function(){ 
            CGstyle = new Array(num_CG) 
            for(let n = 0; n < num_CG; n++){
  //              let CGv = sim.rng.genrand_real1() 
                let CGv = Math.random() 
                console.log('208', CGv) 
                if(CGv < CGp){
                    CGstyle[n] = 0 
                ...
                // if(sim.rng.genrand_real1() < CGp){
                //     CGstyle[n] = 0 
                //     console.log('208', CGstyle) 
                // }
                //     else if(sim.rng.genrand_real1() > CGp){
                //         CGstyle[n] = 1 
                //         console.log('212', CGstyle) 
                //     }
                //         else if(sim.rng.genrand_real1() == CGp){
                //             CGstyle[n] = 1 
                //         }
            }
             console.log('219', CGstyle) 
        }
    ...
    sim.catalyze = function(i,j){
        for(let n = 0; n < num_CG; n++){
            if(CGstyle[n] == 0){
                // console.log('289line', CGstyle) 
                if(sim.dopple.grid[i][j].CGnumbersarray[n] > sim.substrate.grid[i][j].substratesarray[n]){
                    sim.production.grid[i][j].productionsarray[n] = sim.production.grid[i][j].productionsarray[n] + sim.substrate.grid[i][j].substratesarray[n] 
                    sim.substrate.grid[i][j].substratesarray[n] = 0 
                }
                    else if (sim.dopple.grid[i][j].CGnumbersarray[n] < sim.substrate.grid[i][j].substratesarray[n]){
                        sim.production.grid[i][j].productionsarray[n] = sim.production.grid[i][j].productionsarray[n] + sim.dopple.grid[i][j].CGnumbersarray[n] 
                        sim.substrate.grid[i][j].substratesarray[n] = sim.substrate.grid[i][j].substratesarray[n] - sim.dopple.grid[i][j].CGnumbersarray[n] 
                    }
                        else if(sim.dopple.grid[i][j].CGnumbersarray[n] == sim.substrate.grid[i][j].substratesarray[n]){
                            sim.production.grid[i][j].productionsarray[n] = sim.production.grid[i][j].productionsarray[n] + sim.substrate.grid[i][j].substratesarray[n] 
                            sim.substrate.grid[i][j].substratesarray[n] = 0 
        ...
        sim.cells.sumPublicGoods = function (gp) {
            ...
            let CG = new Array(num_CG).fill(0) 
                ...
                if(i > 0 && j > 0 && i < sim.cells.nc && j < sim.cells.nr){
                                for(let n = 0 ; n < num_CG ; n++) { 
                                    if(CGstyle[n] == 0){
                                    // console.log('312 line %d  %d', i,j) 
                                        let realCGNum = sim.production.grid[i][j].productionsarray[n] 
                                        if(realCGNum > 0) {
                                            if(CG[n] == 0) {
                                                CG[n] = 1 
                                            }
        ...
        sim.mutation_function = function(i,j){ 
            ...
            if (sim.cells.grid[i][j].alive == 1 && sim.cells.grid[i][j].genome != undefined) { // This if conditional is simulating the creation of a cheater. 
                for (let t = 0; t < num_CG; t++) { // Let t traverse every public good so that the genomes of every public good have a chance of being lost. 
                    let rng = this.rng.genrand_real1() // Generates a random real number on the [0,1] interval. 
        ...
        sim.cells.consume = function (gp){ 
            ...
            let j = i_j[1]
                for(let z = 0; z < num_CG; z++) {
                        if(CGstyle[z] == 0){
                            if(sim.production.grid[i][j].productionsarray[z] > 0 && this.rng.genrand_real1() < consume_chance){ 
                                sim.production.grid[i][j].productionsarray[z] = Math.floor((sim.production.grid[i][j].productionsarray[z] - consume_number)) 
                            }
                        }
                        if(CGstyle[z] == 1){
        ...
        sim.pro_decay = function(){ 
            if(pro_decay == 1){
                for (let i = 0; i < sim.production.nc; i++) {
                    for (let j = 0; j < sim.production.nr; j++) { 
                        for(let n = 0; n < num_CG; n++){
                            if (sim.production.grid[i][j].productionsarray[n] > 0) {  
                                let cnt = 0     
                                for(let pp = 0; pp < sim.production.grid[i][j].productionsarray[n]; pp++){
                                    if (this.rng.genrand_real1() > prodecay_chance) { 
                                        cnt = cnt + 1 
                                        // console.log("aaa") 
                                    }
                                }
                                sim.production.grid[i][j].productionsarray[n] = cnt 
                                // console.log("aaa line 469") 
                            }
                        } 
                    }
                }            
                ...
                // console.log(sum_resources) 
                    ...
                    for (let bb = 0; bb < random_neighbours.length; bb++) { // This loop simulates the replication of these neighbors, and the scope of the loop is all the neighbors in Moore neighbourhood. 
                        ...
                        if(sum_resources == num_CG){ 
                            ...
                            if (sim.cells.rng.genrand_real1() < repro_chance && sim.cells.grid[ii][jj].alive == 1) { // Generates a random real number on the [0,1] interval. If the real number is smaller than repro_chance, the neighbor can copy itself. 
                               cease = 1 
      ...
      //                          console.log(temp_var) 
                                ...
                                // for(let subnumber = 0; subnumber < num_CG; subnumber++) {
                                //         sim.substrate.grid[i][j].substratesarray[subnumber] = Math.floor((Math.random()*maxInitsubstrateNum)+1)    
                                // } 
                            // console.log('111') 
    ...
    sim.production.writeFirstProduct = function() {
        let filn = dir+"/Product_"+"productionganger_"+filn_suffix+".txt"	
        fs.appendFileSync(filn, `TimeSteps\t`)	
        for(let i = 0; i < num_CG;i++) {
            let num = i + 1 
            fs.appendFileSync(filn, 'Product'+ num +'\t')
        } 
        fs.appendFileSync(filn, `\n`) 
    } 

    sim.production.writeProduct = function() {
        let filn = dir+"/Product_"+"productionganger_"+filn_suffix+".txt" 
        fs.appendFileSync(filn, `${this.time}\t`)	
        CG_one_sum = 0
        CG_two_sum = 0
        ...
        for (let i = 0; i < sim.production.nc; i++) {
            for (let j = 0; j < sim.production.nr; j++) {
                temp_cg = sim.production.grid[i][j] 
                //这个地方要看一下 怎么拿出来产品总数 
                //run 
                for(let num = 0; num < num_CG; num++) {
                    //有空的要判断
                    if(temp_cg.productionsarray){
                        CG_NUMS_ARR[num] = CG_NUMS_ARR[num] + temp_cg.productionsarray[num]
                    }  
                }
            }
        }
    ...
    sim.production.writeGrid = function() {
        filn = dir+"/GridStates_"+"productionganger_"+filn_suffix+"/"+this.time+".txt"	
        if(fs.existsSync(filn)){
            fs.unlinkSync(filn)
        }
        fs.appendFileSync(filn, "x\ty\tproductionsarray\n") 
        for (let i = 0; i < this.nc; i++) {
            for (let j = 0; j < this.nr; j++) {
                
                let point_state = i+'\t'+j+'\t'+this.grid[i][j].productionsarray+'\n'
                 fs.appendFileSync(filn,point_state)	
             }
         }	
    }
       ...
       sim.substrate.initialise() 
       sim.production.initialise() 
       sim.cells.writeGrid() 
       sim.CGtypes() 
       ...
       First_makePermCounts()		
        ...
        //  sim.dopple.writeFirstCG() 
         sim.production.writeFirstProduct() 
       ...
       sim.cells.nextState = function (i, j) { 
        // console.log('sim.cells.nextState %d  %d',i,j) 
               if (this.rng.genrand_real1() < global_death) { // This if conditional is simulating cell death. 
        ...
        sim.substrate.nextState = function (i, j) {  
        } 
        sim.production.nextState = function (i, j) {  
            sim.catalyze(i,j)
        } 
    ...
    sim.substrate.update = function () {
        this.synchronous()         
        if(this.time % subproduct_interval == 0){
            sim.productsub_function() 
            // console.log(1) 
        } 
    }      

    sim.production.update = function () {
        this.synchronous()         
            if(this.time % prodiffuse_interval == 0){
                sim.proconcentation_function() 
                // console.log(2) 
            } 
                if (this.time % 200 == 0){
                    sim.production.writeProduct() 
                    sim.production.writeGrid() 
                } 
                if(this.time % prodecay_interval == 0){
                    sim.pro_decay() 
                }
    }      
    ...
    sim.proconcentation_function = function(){ 
        for (let i = 0; i < sim.production.nc; i++) {
            for (let j = 0; j < sim.production.nr; j++) { 
                for (let z = 0; z < num_CG; z++){
                    if(sim.production.grid[i][j].productionsarray[z] > 0) { 
                        let temp_8array = new Array() 
                        for(let row = i - 1; row <= i + 1; row++){
                            for(let col = j - 1; col <= j + 1; col++) { 
                                if(row < 0){
                                ...
                                if(row >= sim.production.nc){
                                    continue 
                                } 
                                if(col < 0){
                                    continue 
                                } 
                                if(col >= sim.production.nr){
                                    continue 
                                } 
                                if(row == i && col == j){
                                    continue
                                } 
                                 temp_8array.push(sim.production.grid[row][col]) 
                            }
                        } 
                        // console.log('741 line %s', temp_8array) 
                        shuffle(temp_8array) 
                        for(let tmp8 = 0; tmp8 <temp_8array.length; tmp8++) {
                            let temp8_grid = temp_8array[tmp8]
                            if(temp8_grid.productionsarray[z] < sim.production.grid[i][j].productionsarray[z]){
                                temp8_grid.productionsarray[z] += 1 
                                sim.production.grid[i][j].productionsarray[z] -=1 
                            }
                        }
                    }
                }
    ...
    sim.productsub_function = function(){ 
        for (let i = 0; i < sim.substrate.nc; i++) {
            for (let j = 0; j < sim.substrate.nr; j++) {
                if(sim.cells.grid[i][j].alive == 1) {
                    for(let sub = 0; sub < num_CG; sub++){
                                sim.substrate.grid[i][j].substratesarray[sub] += 1
                    }
                }
            }
        } 
